...
# Traitements fichiers xml MESH ----
  # [SORTIE] : 

traitements_xml_mesh <- function(full_xml_database_mesh){
  
# init tableau final
  tableau_dt_final <- data.frame()
  

  nb_elements_xml<- xml_length(full_xml_database_mesh)
  ...
  for(i_file in seq(fic_debut, fic_fin)){
    ...
    file_unique_xml <- xml_child(full_xml_database_mesh, i_file)
    ...
    # TAG ID MESH
    id_unique_file_mesh <- tableau_dt2 %>% 
      filter(Nom_balise %in% "DescriptorUI"	& Parent %in% "DescriptorRecord") %>% 
      pull(Valeur)
    
    tableau_dt2 <- tableau_dt2 %>% 
      mutate(id_mesh= id_unique_file_mesh)
    ...
    saveRDS(tableau_dt_final, paste0(chemin_mesh_rds, "mesh_lot_", i_lot, ".RDS"))
