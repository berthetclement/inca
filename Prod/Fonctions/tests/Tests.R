...
check_data_supp <- function(dt, horodatage, source_file){
  ...
  dt_check = (dt %>% group_by(id_supp) 
                ...
                %>% mutate(Test_Nbrows = Nbrows > 1, Test_Min_depth = Min_depth == 2, Date_Traitement = horodatage, XML_Source = source_file)
...
check_data_desc <- function(dt, horodatage, source_file){
  ...
  dt_check = (dt %>% group_by(id_desc) 
                %>% summarise(Nbrows = n())
                %>% mutate(Test_Nbrows = Nbrows > 1, Date_Traitement = horodatage, XML_Source = source_file)
		        %>% select(-Nbrows)
	        )
